...
async fn main() {
    ...
    let server = Server::bind(&"0.0.0.0:7073".parse().unwrap()).serve(router.into_make_service());
...
async fn realtime_cpus_get(
    ...
    ws.on_upgrade(move |ws: WebSocket| async { realtime_cpus_stream(state, ws).await })
}
